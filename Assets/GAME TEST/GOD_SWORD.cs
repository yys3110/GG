		...
		if(Physics.Raycast(ray , out hit ,Mathf.Infinity)){
			if(Input.GetAxis("Mouse ScrollWheel")>0){
				if(hit.collider.GetComponent<player>() == true){
					hit.collider.GetComponent<player>().HP_system(damage,critical,null,1);
				}
				if(hit.collider.GetComponent<monster>() == true){
					hit.collider.GetComponent<monster>().HP_system(damage,critical,null,1);
				}
			}
			if(Input.GetAxis("Mouse ScrollWheel")<0){
				if(hit.collider.GetComponent<player>() == true){
					hit.collider.GetComponent<player>().HP_system(0,critical,null,1);
					hit.collider.GetComponent<player>().hp_ +=1;
				}
				if(hit.collider.GetComponent<monster>() == true){
					hit.collider.GetComponent<monster>().HP_system(0,critical,null,1);
